...
class Zip
{
    ...
    public function addFile(string $PathAndFile,string $fileName)
    {
      $this->zip->addFile($PathAndFile,$fileName);
    }
    ...
    public function addFiles(array $arrFileNames)
    {
        ...
        foreach ($arrFileNames as $fileName) {
            if (file_exists($fileName)) {
                $rutaArchivos = pathinfo($fileName);
                $this->addFile($fileName, $rutaArchivos['basename']);
            } else {
